...
class LiarDieTrainer {
    ...
    LiarDieTrainer(int sides) {
        this->sides = sides;
        this->responseNodes.resize(sides);
        ...
        for (int myClaim = 0; myClaim <= sides; myClaim++) {
            ...
            for (int oppClaim = myClaim + 1; oppClaim <= sides; oppClaim++)
                this->responseNodes[myClaim][oppClaim] = new Node((oppClaim == 0 || oppClaim == sides) ? 1 : 2);
        }
    ...
    void train(int iterations) {
        ...
        //     for (float prob : claimNodes[0][initialRoll].getAverageStrategy())
